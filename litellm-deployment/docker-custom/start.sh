...
# Check if litellm.env exists
if [ ! -f "litellm.env" ]; then
    echo -e "${RED}Error: litellm.env not found!${NC}"
    echo -e "${YELLOW}Please create a litellm.env file with your configurations.${NC}"
    echo -e "${YELLOW}See litellm.env for an example.${NC}"
    exit 1
fi

# Load environment variables from litellm.env
echo -e "${GREEN}Loading configuration from litellm.env...${NC}"
set -a
source litellm.env
set +a
...
if [ -z "$DATABASE_URL" ]; then
    echo -e "${RED}Error: DATABASE_URL not set in litellm.env!${NC}"
    echo -e "${YELLOW}Please set DATABASE_URL in your litellm.env file.${NC}"
    echo -e "${YELLOW}Example:${NC}"
    echo -e "${GREEN}DATABASE_URL=postgresql://user:password@host:port/database${NC}"
