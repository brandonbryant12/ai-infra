        ...
        try:
            ...
            if existing_trace_id is not None:
                ...
                if "input" in update_trace_keys:
                    trace_params["input"] = "redacted-by-litellm"
                if "output" in update_trace_keys:
                    trace_params["output"] = "redacted-by-litellm"
...
_langfuse_logger = None

def get_langfuse_logger():
    """Get or create the global LangFuse logger instance"""
    global _langfuse_logger
    if _langfuse_logger is None:
        _langfuse_logger = LangFuseLogger()
    return _langfuse_logger

# Custom callback functions for LiteLLM
def langfuse_success_callback(kwargs, completion_response, start_time, end_time):
    """Success callback for Langfuse logging"""
    try:
        logger = get_langfuse_logger()
        return logger.log_event_on_langfuse(
            kwargs=kwargs,
        ...
        print(f"Langfuse success callback error: {e}")
        return {}

def langfuse_failure_callback(kwargs, completion_response, start_time, end_time):
    """Failure callback for Langfuse logging"""
    try:
        logger = get_langfuse_logger()
        return logger.log_event_on_langfuse(
            kwargs=kwargs,
        ...
        print(f"Langfuse failure callback error: {e}")
        return {}